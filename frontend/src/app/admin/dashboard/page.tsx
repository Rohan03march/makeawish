...
export default function AdminDashboard() {
    ...
    const [timeFilter, setTimeFilter] = useState("all") // 'all' | '7days' | 'custom'
    const [date, setDate] = useState<Date | undefined>(new Date())
    const itemsPerPage = 5

    const calculateStats = (currentOrders: any[]) => {
        let filteredStatsOrders = currentOrders

        if (timeFilter !== 'all') {
            const cutoff = new Date()
            if (timeFilter === '7days') cutoff.setDate(cutoff.getDate() - 7)
            if (timeFilter === '10days') cutoff.setDate(cutoff.getDate() - 10)
            if (timeFilter === '30days') cutoff.setDate(cutoff.getDate() - 30)
            ...
            if (timeFilter === 'custom' && date) {
                ...
                filteredStatsOrders = currentOrders.filter((o: any) => {
                    const orderDate = new Date(o.createdAt)
                    return orderDate >= startOfDay && orderDate <= endOfDay
                })
            } else if (timeFilter !== 'custom') {
                filteredStatsOrders = currentOrders.filter((o: any) => new Date(o.createdAt) >= cutoff)
            }
        }

        const totalRevenue = filteredStatsOrders.reduce((acc: number, order: any) => acc + (order.totalPrice || 0), 0)
        const totalOrders = filteredStatsOrders.length
        // Active customers logic might need users data, but for now we approximate from orders or keep it simple
        const uniqueCustomers = new Set(filteredStatsOrders.map((o: any) => o.user?._id || o.user)).size
        ...
        setStats([
            ...
            { label: "Active Customers", value: uniqueCustomers.toString(), change: "+0%", trend: "up", icon: Users, sparkline: [20, 22, 25, 28, 30, 35, 38] },
    ...
    const fetchDashboardData = async (showLoading = false) => {
        if (showLoading) setIsRefetching(true)
        try {
            ...
            if (showLoading) toast.success("Dashboard updated")
            ...
            if (showLoading) setIsRefetching(false)
        }
    }
    ...
    const filteredOrders = orders.filter((order: any) => {
        ...
        const displayId = `ORD-${order._id.substring(order._id.length - 5).toUpperCase()}`
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                ...
                <div className="flex items-center gap-3">
                        ...
                        {timeFilter === 'custom' && (
                            <Popover>
                                <PopoverTrigger asChild>
                                    <button
                                        className={cn(
                                            "flex items-center gap-2 px-4 py-2 bg-chocolate-800/50 hover:bg-chocolate-800 text-chocolate-200 rounded-lg transition-all border border-white/5 text-sm font-medium",
                    ...
                    <button
                        ...
                        className="p-2 bg-chocolate-800/50 hover:bg-chocolate-800 text-chocolate-200 rounded-lg transition-all border border-white/5"
                        disabled={isRefetching}
                    >
                        <RefreshCw className={cn("w-4 h-4", isRefetching && "animate-spin text-gold-400")} />
                    </button>
                </div>
            </div>
